...
for j in eachindex(years)
    ...
    for i in eachindex(ds["time"])#ProgressBar(eachindex(ds["time"]))
        tval = ds[var][:, :, i]
...
using HDF5
h5file = h5open("era5_monthly_total_tprecip_1959_2002.h5", "w") 
write(h5file, "monthly_total_precip", monthly_averaged_var)
